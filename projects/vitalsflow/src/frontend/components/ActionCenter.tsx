    ...
    <div className="glass-card flex flex-col h-full overflow-hidden">
      ...
      <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: "1px solid var(--border-subtle)" }}>
        <div className="flex items-center gap-2">
          <div className="flex h-6 w-6 items-center justify-center rounded-full" style={{ background: "var(--accent-blue-dim)" }} aria-hidden="true">
            <Zap className="h-3.5 w-3.5" style={{ color: "var(--accent-blue)" }} />
          </div>
          <span className="font-semibold text-sm" style={{ color: "var(--text-primary)", fontFamily: "var(--font-outfit)" }}>
            AI Suggested Actions
          </span>
        </div>
        <span className="text-[10px] font-mono tabular-nums rounded-full px-2.5 py-0.5" style={{ background: "var(--bg-elevated)", border: "1px solid var(--border-subtle)", color: "var(--text-secondary)" }}>
      ...
      {/* Progress */}
      {totalCount > 0 && (
        <div className="px-5 py-2 flex items-center gap-3" style={{ borderBottom: "1px solid var(--border-subtle)" }}
          role="status" aria-live="polite" aria-label={`${approvedCount} of ${totalCount} actions approved`}>
          <div className="flex-1 rounded-full overflow-hidden" style={{ height: "4px", background: "var(--bg-elevated)" }} aria-hidden="true">
            <div className="h-full rounded-full transition-all duration-700"
              style={{ width: `${(approvedCount / totalCount) * 100}%`, background: "linear-gradient(90deg, #16a34a, #22c55e)" }} />
          </div>
          <span className="text-[10px] tabular-nums shrink-0" style={{ color: "var(--text-tertiary)" }} aria-hidden="true">
      ...
      <div className="flex flex-col gap-3 p-5 flex-1 overflow-y-auto">
            ...
            {actions.map((action, i) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="flex-1 text-xs" style={{ color: isApproved ? "#15803d" : isDismissed ? "var(--text-muted)" : "var(--text-secondary)" }}>
                    {action}
                  </span>
        ...
        <p className="flex items-start gap-1.5 text-[9px] mt-1 pt-3"
